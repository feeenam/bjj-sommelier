...
import { useLibraryVideo, useCollections, useTags, createCollection, createTag, updateLibraryVideo, Tag } from '../data/library'
...
export function EditLibraryVideoPage() {
  const { id } = useParams<{ id: string }>()
  const navigate = useNavigate()
  const { video, loading: videoLoading } = useLibraryVideo(id)
  ...
  useEffect(() => {
    if (video && !initialized) {
      ...
      setCollectionId(video.collectionId)
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!id || !form.title.trim() || !form.source_url.trim()) return
  ...
  if (videoLoading) {
    return (
      <div className="min-h-screen bg-bjj-bg flex items-center justify-center text-bjj-textMuted">
        Loading...
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-bjj-bg py-12">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
          <h1 className="font-heading text-5xl text-white mb-4">EDIT VIDEO</h1>
          <p className="text-bjj-textMuted text-lg">
            Update video details and organization.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          {success && (
            <div className="flex items-center gap-3 bg-green-500/10 border border-green-500/30 rounded-xl px-5 py-4 text-green-400">
              ...
              <p>Video updated! Redirecting to library...</p>
            </div>
          )}
          {error && (
